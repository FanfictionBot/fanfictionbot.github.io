(function (window, document, undefined) {
  ...
  var createHTML = function (fic) {
    var desc = "Could not get description.";
    if (fic.id in data["fic_id_to_desc"]) {
      desc = DOMPurify.sanitize(data["fic_id_to_desc"][fic.id]);
    }
  ...
  var search = function (query) {
    ...
    resultList.innerHTML =
      results.length < 1 ? createNoResultsHTML() : createResultsHTML(results);
  };
